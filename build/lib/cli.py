...
def _get_master_key(db_path: str) -> MasterKey:
    salt = get_vault_salt(db_path)
    if not salt:
        print("No vault salt found. Run 'init' first.")
        sys.exit(1)
    password = getpass("Enter your master password: ")
    return Authenticator.derive_key(password, salt)


def cmd_add(db_path: str) -> int:
    key = _get_master_key(db_path)
    ...
    nonce, ciphertext, tag = engine.encrypt(secret)
...
def cmd_view(db_path: str, entry_id: int) -> int:
    ...
    nonce = Nonce(row.get("nonce"))
    ciphertext = Ciphertext(row.get("ciphertext"))
    tag = Tag(row.get("tag"))

    try:
        plaintext = engine.decrypt(nonce, ciphertext, tag)
    except InvalidTag:
