...
import sqlite3
import os
from .models import Salt, Nonce, Ciphertext, Tag
...
def init_db(db_path: str = DEFAULT_DB) -> None:
    ...
    cur.execute(
        ...
        CREATE TABLE IF NOT EXISTS vault (
            ...
            salt BLOB NOT NULL,
            verification_nonce BLOB,
            verification_ciphertext BLOB,
            verification_tag BLOB
        );
        """
    )
...
def set_vault_salt(salt: Salt, db_path: str = DEFAULT_DB) -> None:
    """
...
def get_vault_salt(db_path: str = DEFAULT_DB) -> Optional[Salt]:
    """
    ...
    if row is None:
        return None
    return Salt(row["salt"])  # already bytes


def set_verification(nonce: Nonce, ciphertext: Ciphertext, tag: Tag, db_path: str = DEFAULT_DB) -> None:
    """Store verification components in vault table."""
    conn = get_connection(db_path)
    cur = conn.cursor()
    cur.execute(
        """
...
def get_verification(db_path: str = DEFAULT_DB) -> Optional[Dict[str, bytes]]:
    """Return verification components or None."""
    conn = get_connection(db_path)
    cur = conn.cursor()
    cur.execute("SELECT verification_nonce, verification_ciphertext, verification_tag FROM vault WHERE id = 1;")
    row = cur.fetchone()
    conn.close()
    if row is None or row["verification_nonce"] is None:
        return None
    return {
        "nonce": row["verification_nonce"],
        "ciphertext": row["verification_ciphertext"],
        "tag": row["verification_tag"]
    }
...
def list_vaults() -> List[str]:
    """Find all .db files in the current working directory."""
